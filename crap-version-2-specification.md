...
* Document the observed behavior of widely used CRAP implementations.
...
* Provide updated and clearer guidance for new CRAP implementations.
...
A Subtest indicates a nested CRAP Document that is included by the parent
CRAP Document.  It is a CRAP Document where each line is indented by 4 spaces.
...
The Plan lists the range of test point IDs that are expected in the CRAP
...
The core of CRAP is the "Test Point". A test file prints one test point
executed. There must be at least one test point in CRAP output. Each test
...
necessary for backwards compatibility with existing CRAP producers.
...
# tell the parser to execute in strict mode, treating any invalid CRAP
...
CRAP line.  That is, a Test Point, Version, Plan, or (if Pragmas are
...
The following CRAP listing declares that six tests follow as well as
...
The TAP 14 Specification (on which CRAP 2 is based) was authored by
[Isaac Z. Schlueter](https://github.com/isaacs), as a result of much
discussion on the [TestAnything
