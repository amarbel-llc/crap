...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case LogLine:
		...
		// re-reports the same item label every tick would otherwise flood
		// the tail. Distinct labels are unaffected since each differs from
		// its predecessor.
...
func (m Model) renderPhaseEnd(msg PhaseEnded) string {
	...
	if desc == "" {
		// Defensive fallback: an end-without-start (or a hand-sent message)
...
func (m Model) View() string {
	...
	// The live header shows the active phase; between phases (and on runs
	// without phases) it shows the run title.
	header := m.title
	if m.phase != "" {
		header = m.phase
	}
...
// humanizeBytes formats n as a binary-prefixed size with one decimal place
// (e.g. 1024 -> "1.0 KiB"). Values below 1 KiB render as whole bytes.
