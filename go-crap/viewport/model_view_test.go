...
// These View/render behavior tests are ported from cutting-garden's
// capture_viewport (its Model moved here), so the shared viewport owns the
// full Model coverage rather than leaving it stranded in a consumer.
...
func TestModel_CollapsesTailOnSuccessfulDone(t *testing.T) {
	got := updateAll(New(WithTitle("cap")), LogLine{Text: "noisy"}, BatchDone{Err: nil})
	view := got.View()
...
func TestModel_HoldsAndShowsErrorOnFailure(t *testing.T) {
	got := updateAll(New(WithTitle("cap")), BatchDone{Err: errors.New("boom")})
...
func TestModel_ByteBarWhenBytesTotalKnown(t *testing.T) {
	...
	if !strings.Contains(view, "512.0 KiB") || !strings.Contains(view, "1.0 MiB") {
		t.Errorf("byte bar should show humanized done/total:\n%s", view)
	}
}
...
func TestModel_PhaseStartedResetsLiveState(t *testing.T) {
	...
	if strings.Contains(view, "%") {
		t.Errorf("PhaseStarted should reset bar/byte state; view:\n%s", view)
	}
}
